...
class STDCHead(FCNHead):
    ...
    def __init__(self, boundary_threshold=0.1, **kwargs):
        ...
        self.register_buffer(
            "laplacian_kernel",
            torch.tensor([-1, -1, -1, -1, 8, -1, -1, -1, -1], dtype=torch.float32, requires_grad=False).reshape(
                (1, 1, 3, 3)
            ),
        )
        self.fusion_kernel = torch.nn.Parameter(
            torch.tensor([[6.0 / 10], [3.0 / 10], [1.0 / 10]], dtype=torch.float32).reshape(1, 3, 1, 1),
            requires_grad=False,
        )
    ...
    def loss_by_feat(self, seg_logits: Tensor, batch_data_samples: SampleList) -> dict:
        ...
        seg_label = self._stack_batch_gt(batch_data_samples).to(self.laplacian_kernel)
        ...
        boudary_targets_pyramid = F.conv2d(boundary_targets_pyramids, self.fusion_kernel)
