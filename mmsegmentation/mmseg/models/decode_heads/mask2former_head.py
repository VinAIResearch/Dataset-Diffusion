...
class Mask2FormerHead(MMDET_Mask2FormerHead):
    ...
    def _seg_data_to_instance_data(self, batch_data_samples: SampleList):
        ...
        for data_sample in batch_data_samples:
            ...
            if len(masks) == 0:
                gt_masks = torch.zeros((0, gt_sem_seg.shape[-2], gt_sem_seg.shape[-1])).to(gt_sem_seg).long()
            else:
    ...
    def predict(self, x: Tuple[Tensor], batch_img_metas: List[dict], test_cfg: ConfigType) -> Tuple[Tensor]:
        ...
        """
        batch_data_samples = [SegDataSample(metainfo=metainfo) for metainfo in batch_img_metas]
