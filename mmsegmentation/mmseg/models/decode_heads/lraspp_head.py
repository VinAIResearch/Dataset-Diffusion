...
class LRASPPHead(BaseDecodeHead):
    ...
    def __init__(self, branch_channels=(32, 64), **kwargs):
        ...
        for i in range(len(branch_channels)):
            self.convs.add_module(f"conv{i}", nn.Conv2d(self.in_channels[i], branch_channels[i], 1, bias=False))
        ...
        self.aspp_conv = ConvModule(
            self.in_channels[-1], self.channels, 1, norm_cfg=self.norm_cfg, act_cfg=self.act_cfg, bias=False
        )
        self.image_pool = nn.Sequential(
            nn.AvgPool2d(kernel_size=49, stride=(16, 20)),
            ConvModule(self.in_channels[2], self.channels, 1, act_cfg=dict(type="Sigmoid"), bias=False),
        )
    ...
    def forward(self, inputs):
        ...
        x = self.aspp_conv(x) * resize(
            self.image_pool(x), size=x.size()[2:], mode="bilinear", align_corners=self.align_corners
        )
        x = self.conv_up_input(x)

        for i in range(len(self.branch_channels) - 1, -1, -1):
            x = resize(x, size=inputs[i].size()[2:], mode="bilinear", align_corners=self.align_corners)
