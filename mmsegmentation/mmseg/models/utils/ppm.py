...
class DAPPM(BaseModule):
    ...
    def __init__(
        ...
        self.scales = ModuleList(
            [ConvModule(in_channels, branch_channels, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg, **conv_cfg)]
        )
            ...
            self.scales.append(
                Sequential(
                    *[
                        ...
                        ConvModule(
                            in_channels, branch_channels, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg, **conv_cfg
                        ),
                    ]
                )
            )
        ...
        self.compression = ConvModule(
            branch_channels * num_scales, out_channels, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg, **conv_cfg
        )

        self.shortcut = ConvModule(
            in_channels, out_channels, kernel_size=1, norm_cfg=norm_cfg, act_cfg=act_cfg, **conv_cfg
        )
    ...
    def forward(self, inputs: Tensor):
        ...
        for i in range(1, self.num_scales):
            feat_up = F.interpolate(
                self.scales[i](inputs), size=inputs.shape[2:], mode=self.unsample_mode, align_corners=False
            )
