...
def parse_args():
    parser = argparse.ArgumentParser(description="Gather benchmarked model evaluation results")
        ...
        for model_info in model_infos:
            ...
            for record_metric_key in previous_metrics:
                if record_metric_key not in metric["metric"]:
                    raise KeyError("record_metric_key not exist, please " "check your config")
                ...
                differential_results[record_metric_key] = f"{flag}{abs(differential):.2f}"
                new_metrics[record_metric_key] = new_metric

            result_dict[config] = dict(differential=differential_results, previous=previous_metrics, new=new_metrics)
