...
def main():
    ...
    assert not (log_items and ignore_keywords), "log_items and ignore_keywords cannot be specified at the same time"
    ...
    if not log_items:
        log_items = [item for item in sorted(os.listdir(work_dir)) if not has_keyword(item, ignore_keywords)]

    experiment_info_list = []
    for config_dir in log_items:
        preceding_path = os.path.join(work_dir, config_dir)
        log_list = [item for item in os.listdir(preceding_path) if item.endswith(".log.json")]
        log_list = sorted(
            log_list, key=lambda time_str: datetime.datetime.strptime(time_str, "%Y%m%d_%H%M%S.log.json")
        )
        val_list = []
        last_iter = 0
        for log_name in log_list:
            with open(os.path.join(preceding_path, log_name)) as f:
                ...
                val_list.extend([json.loads(line) for line in all_lines if json.loads(line)["mode"] == "val"])
        ...
        assert len(val_list) >= 1, f"work dir {config_dir} doesn't contain any evaluation."
    ...
    if markdown_file:
        ...
        with open(markdown_file, "w") as f:
            f.write(f"|exp_num|method|{metric} best|best index|" f"{metric} last|last index|last iter num|\n")
            f.write("|:---:|:---:|:---:|:---:|:---:|:---:|:---:|\n")
