...
def get_model_info(md_file: str, config_dir: str, collection_name_list: List[str]) -> Tuple[dict, str]:
        ...
        while i < len(lines):
            ...
            if (
                ...
                while j < len(lines) and lines[j][0] == "|":
                    ...
                    # get config name
                    try:
                        config_url = re.findall(r"[a-zA-Z]+://[^\s]*py", values[config_idx])[0]
                        config_name = config_url.split("/")[-1]
                        model_name = config_name.replace(".py", "")
                    except IndexError:
                        raise ValueError(f"config url is not found in {md_file}")

                    # get model name
                    try:
                        weight_url = re.findall(r"[a-zA-Z]+://[^\s]*pth", values[download_idx])[0]
                        log_url = re.findall(r"[a-zA-Z]+://[^\s]*.json", values[download_idx + 1])[0]
                    except IndexError:
                        raise ValueError(f"url is not found in {values[download_idx]}")

                    # get batch size
                    bs = re.findall(r"[0-9]*xb[0-9]*", config_name)[0].split("xb")
                    ...
                    mem = values[mem_idx].split("\\")[0] if values[mem_idx] != "-" and values[mem_idx] != "" else -1
                    ...
                    if " + " in method:
                        ...
                        method = [method]
                    backone: str = re.findall(r"[^\s]*", values[keys.index("Backbone")].strip())[0]
                    archs = [backone] + method
                    collection_name = method[0]
                    config_path = osp.join("configs", config_dir.split("/")[-1], config_name)
                    model = {
                        "Name": model_name,
                        ...
                        "Metadata": {
                            ...
                            "Architecture": archs,
                            "Training Resources": f"{bs[0]}x {values[device_idx]} GPUS",
                        },
                        "Weights": weight_url,
                        "Training log": log_url,
                        "Paper": {"Title": paper_name, "URL": paper_url},
                        "Code": code_url,
                        "Framework": "PyTorch",
                    }
                    if ms_idx != -1 and values[ms_idx] != "-" and values[ms_idx] != "":
                        model["Results"]["Metrics"].update({keys[ms_idx]: float(values[ms_idx])})
                    if mem != -1:
    ...
    if not (is_dataset or is_backbone) or collection_name not in collection_name_list:
        collection = {
            "Name": collection_name,
            "License": "Apache License 2.0",
            "Metadata": {"Training Data": datasets},
            "Paper": {
                "Title": paper_name,
                "URL": paper_url,
            },
            "README": osp.join("configs", config_dir.split("/")[-1], "README.md"),
            "Frameworks": ["PyTorch"],
        }
        results = {"Collections": [collection], "Models": models}, collection_name
    else:
...
def update_model_index(config_dir_list: List[str]) -> bool:
    """update model index."""
    yml_files = [osp.join("configs", dir_name.split("/")[-1], "metafile.yaml") for dir_name in config_dir_list]
    yml_files.sort()

    model_index = {"Import": [osp.relpath(yml_file, MMSEG_ROOT).replace("\\", "/") for yml_file in yml_files]}
...
if __name__ == "__main__":
    ...
    for md_file, config_dir in zip(md_file_list, config_dir_list):
        results, collection_name = get_model_info(md_file, config_dir, collection_name_list)
