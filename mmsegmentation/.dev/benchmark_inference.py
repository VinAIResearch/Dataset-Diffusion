...
def parse_args():
    ...
    parser.add_argument("-d", "--device", default="cuda:0", help="Device used for inference")
...
def main(args):
    ...
    # test all model
    logger = get_root_logger(log_file="benchmark_inference_image.log", log_level=logging.ERROR)
        ...
        for model_info in model_infos:
            ...
            try:
                if not osp.exists(checkpoint):
                    download_checkpoint(checkpoint_name, model_name, config_name.rstrip(".py"), args.checkpoint_root)
            except Exception:
