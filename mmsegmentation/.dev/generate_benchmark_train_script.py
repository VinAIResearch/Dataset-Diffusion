...
def parse_args():
    ...
    parser.add_argument(
        "--out", type=str, default=".dev/benchmark_train.sh", help="path to save model benchmark script"
    )
...
def create_train_bash_info(commands, config, script_name, partition, port):
    ...
    command_info += f"{cfg} "
    command_info += f"--cfg-options " f"checkpoint_config.max_keep_ckpts=1 " f"dist_params.port={port} "
