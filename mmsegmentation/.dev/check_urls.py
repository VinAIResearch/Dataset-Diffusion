...
def parse_args():
    ...
    parser.add_argument("-m", "--model-name", type=str, help="Select the model needed to check")
...
def main():
    ...
    if model_name is not None:
        ...
        # check all
        yml_list = [(x, f"configs/{x}/{x}.yml") for x in os.listdir("configs/") if x != "_base_"]
    ...
    for model_name, yml_path in yml_list:
        ...
        for model_info in model_infos:
            ...
            if flag:
                logger.info(f"checkpoint | {config_name} | {checkpoint_url} | " f"{status_code} valid")
            else:
                logger.error(f"checkpoint | {config_name} | {checkpoint_url} | " f"{status_code} | error")
            # log_json check
            checkpoint_name = checkpoint_url.split("/")[-1]
            model_time = "-".join(checkpoint_name.split("-")[:-1]).replace(f"{config_name}_", "")
            ...
            if flag_1 or flag_2:
                if flag_1:
                    logger.info(f"log.json | {config_name} | {log_json_url_1} | " f"{status_code_1} | valid")
                else:
                    logger.info(f"log.json | {config_name} | {log_json_url_2} | " f"{status_code_2} | valid")
            else:
                logger.error(
