...
def parse_args():
    ...
    parser.add_argument("-f", "--config-name", type=str, help="Process the selected config.")
    parser.add_argument(
        "-w",
        "--work-dir",
        default="work_dirs/",
    ...
    parser.add_argument("--all", action="store_true", help="whether include .py and .log")
...
def main():
    ...
    for model in model_infos:
        ...
        publish_model_path = osp.join(model_publish_dir, config_name + "_" + model["model_time"])
        trained_model_path = osp.join(work_dir, config_name, "iter_{}.pth".format(model["iters"]))
        ...
        # copy log
        shutil.copy(
            osp.join(work_dir, config_name, model["log_json_path"]), osp.join(model_publish_dir, new_json_path)
        )
        ...
        if args.all:
            ...
            Config.fromfile(raw_config).dump(osp.join(model_publish_dir, osp.basename(raw_config)))
