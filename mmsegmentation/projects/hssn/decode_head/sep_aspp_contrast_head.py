...
class DepthwiseSeparableASPPContrastHead(DepthwiseSeparableASPPHead):
    ...
    def __init__(self, proj: str = "convmlp", **kwargs):
        ...
        self.proj_head = ProjectionHead(dim_in=2048, norm_cfg=self.norm_cfg, proj=proj)
    ...
    def predict_by_feat(self, seg_logits: Tuple[Tensor], batch_img_metas: List[dict]) -> Tensor:
        ...
        seg_logit = resize(
            input=seg_logit, size=batch_img_metas[0]["img_shape"], mode="bilinear", align_corners=self.align_corners
        )

        return seg_logit

    def loss_by_feat(self, seg_logits: Tuple[Tensor], batch_data_samples: SampleList) -> dict:  # (out, embedding)
        ...
        loss = dict()
        seg_logit = resize(
            input=seg_logit_before, size=seg_label.shape[2:], mode="bilinear", align_corners=self.align_corners
        )
        ...
        seg_logit_before = resize(
            input=seg_logit_before, scale_factor=0.5, mode="bilinear", align_corners=self.align_corners
        )
