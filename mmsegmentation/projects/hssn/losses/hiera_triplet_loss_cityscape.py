...
def losses_hiera(predictions, targets, targets_top, num_classes, indices_high, eps=1e-8):
    ...
    for ii in range(7):
        for jj in range(indices_high[ii][0], indices_high[ii][1]):
            MCLA[:, jj : jj + 1, :, :] = torch.min(
                torch.cat([predictions[:, jj : jj + 1, :, :], MCLB[:, ii : ii + 1, :, :]], dim=1), 1, True
            )[0]
    ...
    loss += (
        (
            (
                -targets_top[:, :, :, :] * torch.log(MCLB + eps)
...
def losses_hiera_focal(predictions, targets, targets_top, num_classes, indices_high, eps=1e-8, gamma=2):
    ...
    loss = (
        (
            (
                -targets[:, :num_classes, :, :] * torch.pow((1.0 - MCLA), gamma) * torch.log(MCLA + eps)
    ...
    loss += (
        (
            (
                -targets_top[:, :, :, :] * torch.pow((1.0 - MCLB), gamma) * torch.log(MCLB + eps)
...
class HieraTripletLossCityscape(nn.Module):
    ...
    def forward(self, step, embedding, cls_score_before, cls_score, label, weight=None, **kwargs):
        ...
        if torch.distributed.get_world_size() == torch.nonzero(class_counts, as_tuple=False).size(0):
            factor = (
                1 / 4 * (1 + torch.cos(torch.tensor((step.item() - 80000) / 80000 * math.pi)))
                if step.item() < 80000
                else 0.5
            )
