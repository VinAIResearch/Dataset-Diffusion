...
class SemanticLevelContext(nn.Module):
    ...
    def __init__(
        self, feats_channels, transform_channels, concat_input=False, conv_cfg=None, norm_cfg=None, act_cfg=None
    ):
        super().__init__()
        self.correlate_net = SelfAttentionBlock(
...
class ISNetHead(BaseDecodeHead):
    ...
    def __init__(
        ...
        **kwargs
        ...
        self.decoder_stage1 = nn.Sequential(
            ...
            nn.Conv2d(self.channels, self.num_classes, kernel_size=1, stride=1, padding=0, bias=True),
        )
    ...
    def forward(self, inputs):
        ...
        preds_stage1 = resize(preds_stage1, size=feats.size()[2:], mode="bilinear", align_corners=self.align_corners)

        feats_sl = self.slc_net(feats, preds_stage1, feats_il)

        if hasattr(self, "shortcut"):
            shortcut_out = self.shortcut(x[0])
            feats_sl = resize(feats_sl, size=shortcut_out.shape[2:], mode="bilinear", align_corners=self.align_corners)
    ...
    def loss_by_feat(self, seg_logits: Tensor, batch_data_samples: SampleList) -> dict:
        ...
        for seg_logit, loss_decode in zip(seg_logits, self.loss_decode):
            ...
            loss[loss_decode.name] = loss_decode(seg_logit, seg_label, seg_weight, ignore_index=self.ignore_index)
