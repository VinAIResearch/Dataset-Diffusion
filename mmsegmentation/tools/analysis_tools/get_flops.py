...
def inference(args: argparse.Namespace, logger: MMLogger) -> dict:
    ...
    data_batch = {"inputs": [torch.rand(input_shape)], "data_samples": [SegDataSample(metainfo=result)]}
    ...
    outputs = get_model_complexity_info(model, input_shape, inputs=data["inputs"], show_table=False, show_arch=False)
