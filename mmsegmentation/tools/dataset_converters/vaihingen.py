...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert vaihingen dataset to mmsegmentation format")
    parser.add_argument("dataset_path", help="vaihingen folder path")
    parser.add_argument("--tmp_dir", help="path of the temporary directory")
    parser.add_argument("-o", "--out_dir", help="output path")
    parser.add_argument("--clip_size", type=int, help="clipped size of image after preparation", default=512)
    parser.add_argument("--stride_size", type=int, help="stride of clipping original images", default=256)
...
def clip_big_image(image_path, clip_save_dir, to_label=False):
    ...
    num_rows = math.ceil((h - cs) / ss) if math.ceil((h - cs) / ss) * ss + cs >= h else math.ceil((h - cs) / ss) + 1
    num_cols = math.ceil((w - cs) / ss) if math.ceil((w - cs) / ss) * ss + cs >= w else math.ceil((w - cs) / ss) + 1
    ...
    boxes = np.stack(
        [xmin + xmin_offset, ymin + ymin_offset, np.minimum(xmin + cs, w), np.minimum(ymin + cs, h)], axis=1
    )

    if to_label:
        color_map = np.array(
            [[0, 0, 0], [255, 255, 255], [255, 0, 0], [255, 255, 0], [0, 255, 0], [0, 255, 255], [0, 0, 255]]
        )
    ...
    for box in boxes:
        ...
        mmcv.imwrite(
            ...
            osp.join(clip_save_dir, f"{area_idx}_{start_x}_{start_y}_{end_x}_{end_y}.png"),
        )
