...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert potsdam dataset to mmsegmentation format")
...
def clip_big_image(image_path, clip_save_dir, args, to_label=False):
    ...
    boxes = np.stack(
        [xmin + xmin_offset, ymin + ymin_offset, np.minimum(xmin + clip_size, w), np.minimum(ymin + clip_size, h)],
        axis=1,
    )
    ...
    for box in boxes:
        ...
        mmcv.imwrite(
            ...
            osp.join(clip_save_dir, f"{idx_i}_{idx_j}_{start_x}_{start_y}_{end_x}_{end_y}.png"),
        )
