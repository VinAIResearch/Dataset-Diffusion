...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert STARE dataset to mmsegmentation format")
...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        ...
        for filename in os.listdir(osp.join(tmp_dir, "gz")):
            un_gz(osp.join(tmp_dir, "gz", filename), osp.join(tmp_dir, "files", osp.splitext(filename)[0]))

        now_dir = osp.join(tmp_dir, "files")

        assert len(os.listdir(now_dir)) == STARE_LEN, f"len(os.listdir(now_dir)) != {STARE_LEN}"
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        ...
        for filename in sorted(os.listdir(now_dir))[:TRAINING_LEN]:
            ...
            mmcv.imwrite(
                img[:, :, 0] // 128, osp.join(out_dir, "annotations", "training", osp.splitext(filename)[0] + ".png")
            )

        for filename in sorted(os.listdir(now_dir))[TRAINING_LEN:]:
            img = mmcv.imread(osp.join(now_dir, filename))
            mmcv.imwrite(
                img[:, :, 0] // 128, osp.join(out_dir, "annotations", "validation", osp.splitext(filename)[0] + ".png")
            )
