...
def main():
    ...
    mkdir_or_exist(osp.join(devkit_path, "VOC2010/ImageSets/SegmentationContext"))

    train_list = track_progress(partial(generate_labels, detail=train_detail, out_dir=out_dir), train_ids)
    with open(osp.join(devkit_path, "VOC2010/ImageSets/SegmentationContext", "train.txt"), "w") as f:
        f.writelines(line + "\n" for line in sorted(train_list))

    val_list = track_progress(partial(generate_labels, detail=val_detail, out_dir=out_dir), val_ids)
