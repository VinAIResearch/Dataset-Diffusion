...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert REFUGE dataset to mmsegmentation format")
...
def extract_img(root: str, cur_dir: str, out_dir: str, mode: str = "train", file_type: str = "img") -> None:
    ...
    for cur_dir, dirs, files in os.walk(root):
        ...
        if len(dirs) == 0 and cur_dir.split("\\")[-1].find("Illustration") == -1 and cur_dir.find("MACOSX") == -1:
            ...
            for filename in sorted(file_names):
                ...
                mmcv.imwrite(img, osp.join(out_dir, file_type, mode, osp.splitext(filename)[0] + ".png"))
