...
def parse_args():
    ...
    parser.add_argument("healthy_manualsegm_path", help="the path of healthy_manualsegm.zip")
    parser.add_argument("glaucoma_path", help="the path of glaucoma.zip")
    parser.add_argument("glaucoma_manualsegm_path", help="the path of glaucoma_manualsegm.zip")
    parser.add_argument("diabetic_retinopathy_path", help="the path of diabetic_retinopathy.zip")
    parser.add_argument("diabetic_retinopathy_manualsegm_path", help="the path of diabetic_retinopathy_manualsegm.zip")
...
def main():
    args = parse_args()
    images_path = [args.healthy_path, args.glaucoma_path, args.diabetic_retinopathy_path]
    annotations_path = [
    ...
    for now_path in images_path:
        with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
            ...
            assert len(os.listdir(tmp_dir)) == HRF_LEN, f"len(os.listdir(tmp_dir)) != {HRF_LEN}"

            for filename in sorted(os.listdir(tmp_dir))[:TRAINING_LEN]:
                img = mmcv.imread(osp.join(tmp_dir, filename))
                mmcv.imwrite(img, osp.join(out_dir, "images", "training", osp.splitext(filename)[0] + ".png"))
            for filename in sorted(os.listdir(tmp_dir))[TRAINING_LEN:]:
                img = mmcv.imread(osp.join(tmp_dir, filename))
                mmcv.imwrite(img, osp.join(out_dir, "images", "validation", osp.splitext(filename)[0] + ".png"))
    ...
    for now_path in annotations_path:
        with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
            ...
            for filename in sorted(os.listdir(tmp_dir))[:TRAINING_LEN]:
                ...
                mmcv.imwrite(
                    img[:, :, 0] // 128,
                    osp.join(out_dir, "annotations", "training", osp.splitext(filename)[0] + ".png"),
                )
            for filename in sorted(os.listdir(tmp_dir))[TRAINING_LEN:]:
                img = mmcv.imread(osp.join(tmp_dir, filename))
                mmcv.imwrite(
                    img[:, :, 0] // 128,
                    osp.join(out_dir, "annotations", "validation", osp.splitext(filename)[0] + ".png"),
                )
