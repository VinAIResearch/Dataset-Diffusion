...
def parse_args():
    parser = argparse.ArgumentParser(description="Convert CHASE_DB1 dataset to mmsegmentation format")
...
def main():
    ...
    with tempfile.TemporaryDirectory(dir=args.tmp_dir) as tmp_dir:
        ...
        assert len(os.listdir(tmp_dir)) == CHASE_DB1_LEN, f"len(os.listdir(tmp_dir)) != {CHASE_DB1_LEN}"
        ...
        for img_name in sorted(os.listdir(tmp_dir))[:TRAINING_LEN]:
            ...
            if osp.splitext(img_name)[1] == ".jpg":
                mmcv.imwrite(img, osp.join(out_dir, "images", "training", osp.splitext(img_name)[0] + ".png"))
            else:
                ...
                mmcv.imwrite(
                    img[:, :, 0] // 128,
                    osp.join(out_dir, "annotations", "training", osp.splitext(img_name)[0] + ".png"),
                )

        for img_name in sorted(os.listdir(tmp_dir))[TRAINING_LEN:]:
            img = mmcv.imread(osp.join(tmp_dir, img_name))
            if osp.splitext(img_name)[1] == ".jpg":
                mmcv.imwrite(img, osp.join(out_dir, "images", "validation", osp.splitext(img_name)[0] + ".png"))
            else:
                mmcv.imwrite(
                    img[:, :, 0] // 128,
                    osp.join(out_dir, "annotations", "validation", osp.splitext(img_name)[0] + ".png"),
                )
