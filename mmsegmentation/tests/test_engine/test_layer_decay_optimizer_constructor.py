...
def test_learning_rate_decay_optimizer_constructor():
    ...
    layerwise_paramwise_cfg = dict(decay_rate=decay_rate, decay_type="layer_wise", num_layers=3)
    optim_wrapper_cfg = dict(
        type="OptimWrapper",
        optimizer=optimizer_cfg,
        paramwise_cfg=layerwise_paramwise_cfg,
    ...
    optim_wrapper = build_optim_wrapper(model, optim_wrapper_cfg)
    check_optimizer_lr_wd(optim_wrapper.optimizer, expected_layer_wise_wd_lr_beit)

    # Test invalidation of lr wd for Vit
    backbone = ToyViT()
    model = PseudoDataParallel(ToySegmentor(backbone))
    with pytest.raises(NotImplementedError):
        optim_constructor = LearningRateDecayOptimizerConstructor(optim_wrapper_cfg, layerwise_paramwise_cfg)
        optim_constructor(model)
    with pytest.raises(NotImplementedError):
        optim_constructor = LearningRateDecayOptimizerConstructor(optim_wrapper_cfg, stagewise_paramwise_cfg)
        optim_constructor(model)
...
def test_beit_layer_decay_optimizer_constructor():
    ...
    optim_wrapper_cfg = dict(
        ...
        optimizer=dict(type="AdamW", lr=1, betas=(0.9, 0.999), weight_decay=0.05),
    )
    optim_wrapper = build_optim_wrapper(model, optim_wrapper_cfg)
