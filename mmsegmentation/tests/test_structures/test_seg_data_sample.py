...
class TestSegDataSample(TestCase):
    def test_init(self):
        meta_info = dict(img_size=[256, 256], scale_factor=np.array([1.5, 1.5]), img_shape=torch.rand(4))
    ...
    def test_setter(self):
        ...
        assert _equal(seg_data_sample.gt_sem_seg.sem_seg, gt_sem_seg_data["sem_seg"])
        ...
        assert _equal(seg_data_sample.pred_sem_seg.sem_seg, pred_sem_seg_data["sem_seg"])
        ...
        assert _equal(seg_data_sample.seg_logits.sem_seg, seg_logits_data["sem_seg"])
