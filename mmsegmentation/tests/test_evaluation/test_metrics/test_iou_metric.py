...
class TestIoUMetric(TestCase):
    def _demo_mm_inputs(self, batch_size=2, image_shapes=(3, 64, 64), num_classes=5):
    ...
    def _demo_mm_model_output(self, data_samples, batch_size=2, image_shapes=(3, 64, 64), num_classes=5):
        _, h, w = image_shapes

        for data_sample in data_samples:
            data_sample["seg_logits"] = dict(data=torch.randn(num_classes, h, w))
            data_sample["pred_sem_seg"] = dict(data=torch.randint(0, num_classes, (1, h, w)))
            data_sample["img_path"] = "tests/data/pseudo_dataset/imgs/00000_img.jpg"
        return data_samples
    ...
    def test_evaluate(self):
        ...
        iou_metric.dataset_meta = dict(
            classes=["wall", "building", "sky", "floor", "tree"], label_map=dict(), reduce_zero_label=False
        )
        ...
        # test format_only
        iou_metric = IoUMetric(iou_metrics=["mIoU"], output_dir="tmp", format_only=True)
        iou_metric.dataset_meta = dict(
            classes=["wall", "building", "sky", "floor", "tree"], label_map=dict(), reduce_zero_label=False
        )
        iou_metric.process([0] * len(data_samples), data_samples)
