...
class TestCityscapesMetric(TestCase):
    def _demo_mm_inputs(self, batch_size=1, image_shapes=(3, 128, 256), num_classes=5):
    ...
    def _demo_mm_model_output(self, batch_size=1, image_shapes=(3, 128, 256), num_classes=5):
        """Create a superset of inputs needed to run test or train batches.
        ...
        seg_pred = np.random.randint(0, num_classes, (batch_size, h, w), dtype=np.uint8)
        seg_pred = torch.LongTensor(seg_pred)
        results_dict["pred_sem_seg"] = seg_pred

        batch_datasampes = [SegDataSample() for _ in range(results_dict["pred_sem_seg"].shape[0])]
    ...
    def test_evaluate(self):
        ...
        data_samples = [dict(**data, **result) for data, result in zip(data_batch, predictions)]
        # test keep_results should be True when format_only is True
        with pytest.raises(AssertionError):
            CityscapesMetric(output_dir="tmp", format_only=True, keep_results=False)
        ...
        # test format_only
        metric = CityscapesMetric(output_dir="tmp", format_only=True, keep_results=True)
        metric.process(data_batch, data_samples)
        metric.evaluate(2)
