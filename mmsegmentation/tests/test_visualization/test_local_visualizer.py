        ...
        def test_add_datasample_forward(gt_sem_seg):
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                seg_local_visualizer = SegLocalVisualizer(
                    vis_backends=[dict(type="LocalVisBackend")], save_dir=tmp_dir
                )
                seg_local_visualizer.dataset_meta = dict(
                    classes=("background", "foreground"), palette=[[120, 120, 120], [6, 230, 230]]
                )

                # test out_file
                seg_local_visualizer.add_datasample(out_file, image, data_sample)

                assert os.path.exists(osp.join(tmp_dir, "vis_data", "vis_image", out_file + "_0.png"))
                drawn_img = cv2.imread(osp.join(tmp_dir, "vis_data", "vis_image", out_file + "_0.png"))
                ...
                self._assert_image_and_shape(
                    osp.join(tmp_dir, "vis_data", "vis_image", out_file + "_0.png"), (h, w * 2, 3)
                )

                seg_local_visualizer.add_datasample(out_file, image, data_sample, draw_gt=False)
                self._assert_image_and_shape(
                    osp.join(tmp_dir, "vis_data", "vis_image", out_file + "_0.png"), (h, w, 3)
                )
        ...
        def test_cityscapes_add_datasample_forward(gt_sem_seg):
            ...
            with tempfile.TemporaryDirectory() as tmp_dir:
                ...
                seg_local_visualizer.add_datasample(
                    out_file, image, data_sample, out_file=osp.join(tmp_dir, "test.png")
                )
                self._assert_image_and_shape(osp.join(tmp_dir, "test.png"), (h, w, 3))
