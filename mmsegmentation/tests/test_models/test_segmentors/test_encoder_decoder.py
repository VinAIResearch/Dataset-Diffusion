...
def test_encoder_decoder():
    ...
    cfg = ConfigDict(
        type="EncoderDecoder",
        backbone=dict(type="ExampleBackbone"),
        decode_head=dict(type="ExampleDecodeHead", num_classes=2, out_channels=1),
        train_cfg=None,
        test_cfg=dict(mode="whole"),
    )
    segmentor = build_segmentor(cfg)
...
def test_postprocess_result():
    ...
    data_sample.gt_sem_seg = PixelData(**{"data": torch.randint(0, 10, (1, 8, 8))})
    data_sample.set_metainfo({"padding_size": (0, 2, 0, 2), "ori_shape": (8, 8)})
    ...
    data_sample = SegDataSample()
    data_sample.gt_sem_seg = PixelData(**{"data": torch.randint(0, 10, (1, 8, 8))})
    data_sample.set_metainfo({"img_padding_size": (0, 2, 0, 2), "ori_shape": (8, 8)})
