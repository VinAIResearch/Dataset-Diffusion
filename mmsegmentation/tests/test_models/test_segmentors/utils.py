...
def _demo_mm_inputs(input_shape=(1, 3, 8, 16), num_classes=10):
    ...
    segs = torch.randint(low=0, high=num_classes - 1, size=(N, H, W), dtype=torch.long)
    ...
    data_samples = [SegDataSample(gt_sem_seg=PixelData(data=segs[i]), metainfo=img_metas[i]) for i in range(N)]
...
class ExampleDecodeHead(BaseDecodeHead):
    def __init__(self, num_classes=19, out_channels=None, **kwargs):
        super().__init__(3, 3, num_classes=num_classes, out_channels=out_channels, **kwargs)
...
def _segmentor_forward_train_test(segmentor):
    ...
    if not hasattr(segmentor, "data_preprocessor") or segmentor.data_preprocessor is None:
