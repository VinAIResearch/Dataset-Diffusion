...
def test_maskformer_head():
    ...
    cfg = Config.fromfile(
        join(repo_dpath, "../../configs/maskformer/maskformer_r50-d32_8xb2-160k_ade20k-512x512.py")  # noqa
    )
    ...
    # test inference
    batch_img_metas = [dict(scale_factor=(1.0, 1.0), img_shape=(512, 683), ori_shape=(512, 683))]
    ...
    for _ in range(2):
        data_sample = SegDataSample(gt_sem_seg=PixelData(data=torch.ones(512, 512).long()))
        data_sample.set_metainfo(img_meta)
        batch_data_samples.append(data_sample)
    train_cfg = {}
    losses = decode_head.loss(inputs, batch_data_samples, train_cfg)
    assert (loss in losses.keys() for loss in ("loss_cls", "loss_mask", "loss_dice"))
