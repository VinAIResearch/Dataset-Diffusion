...
def test_setr_mla_head(capsys):
    ...
    with pytest.raises(AssertionError):
        ...
        SETRMLAHead(in_channels=(8, 8, 8, 8), channels=8, mla_channels=4, in_index=(0, 1, 2, 3), num_classes=19)
    ...
    x = [torch.randn(1, 8, h, w), torch.randn(1, 8, h, w), torch.randn(1, 8, h, w), torch.randn(1, 8, h, w)]
