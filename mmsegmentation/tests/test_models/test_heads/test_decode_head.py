...
def test_decode_head():
    ...
    data_samples = [SegDataSample(gt_sem_seg=PixelData(data=torch.ones(64, 64).long())) for _ in range(2)]

    head = BaseDecodeHead(
        3, 16, num_classes=19, loss_decode=dict(type="CrossEntropyLoss", use_sigmoid=False, loss_weight=1.0)
    )
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    loss = head.loss_by_feat(seg_logits=inputs, batch_data_samples=data_samples)
    ...
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    loss_3 = head.loss_by_feat(seg_logits=inputs, batch_data_samples=data_samples)

    head = BaseDecodeHead(3, 16, num_classes=19, loss_decode=(dict(type="CrossEntropyLoss", loss_name="loss_ce")))
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
    loss = head.loss_by_feat(seg_logits=inputs, batch_data_samples=data_samples)
    assert "loss_ce" in loss
