...
def test_setr_up_head(capsys):
    ...
    head = SETRUPHead(
        in_channels=4, channels=2, norm_cfg=dict(type="SyncBN"), num_classes=19, init_cfg=dict(type="Kaiming")
    )
    ...
    patch_size = 2
    head = SETRUPHead(
        in_channels=4, channels=2, num_classes=19, num_convs=1, up_scale=4, kernel_size=1, norm_cfg=dict(type="BN")
    )
