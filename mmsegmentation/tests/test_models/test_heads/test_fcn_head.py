...
def test_fcn_head():
    ...
    # test with norm_cfg
    head = FCNHead(in_channels=8, channels=4, num_classes=19, norm_cfg=dict(type="SyncBN"))
    for m in head.modules():
        if isinstance(m, ConvModule):
    ...
    # test num_conv = 0
    inputs = [torch.randn(1, 8, 23, 23)]
    head = FCNHead(in_channels=8, channels=8, num_classes=19, num_convs=0, concat_input=False)
    if torch.cuda.is_available():
        head, inputs = to_cuda(head, inputs)
