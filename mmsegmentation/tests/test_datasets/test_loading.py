...
class TestLoading:
    ...
    def test_load_seg(self):
        ...
        results = dict(seg_map_path=seg_path, reduce_zero_label=True, seg_fields=[])
    ...
    def test_load_seg_custom_classes(self):
        ...
        results = dict(
            ...
            label_map={0: 0, 1: 255, 2: 1, 3: 2},  # simulate removing class 1
        ...
        results = dict(img_path=img_path, seg_map_path=gt_path, reduce_zero_label=False, seg_fields=[])
    ...
    def test_load_biomedical_img(self):
        results = dict(img_path=osp.join(self.data_prefix, "biomedical.nii.gz"))
        transform = LoadBiomedicalImageFromFile()
        results = transform(copy.deepcopy(results))
        assert results["img_path"] == osp.join(self.data_prefix, "biomedical.nii.gz")
    ...
    def test_load_biomedical_annotation(self):
        results = dict(seg_map_path=osp.join(self.data_prefix, "biomedical_ann.nii.gz"))
    ...
    def test_load_biomedical_data(self):
        input_results = dict(img_path=osp.join(self.data_prefix, "biomedical.npy"))
        transform = LoadBiomedicalData(with_seg=True)
        results = transform(copy.deepcopy(input_results))
        assert results["img_path"] == osp.join(self.data_prefix, "biomedical.npy")
        assert results["img"][0].shape == results["gt_seg_map"].shape
        assert results["img"].dtype == np.float32
        assert results["ori_shape"] == results["img"].shape[1:]
        assert repr(transform) == (
            "LoadBiomedicalData(" "with_seg=True, " "decode_backend='numpy', " "to_xyz=False, " "backend_args=None)"
        )
        ...
        assert repr(transform) == (
            "LoadBiomedicalData(" "with_seg=False, " "decode_backend='numpy', " "to_xyz=False, " "backend_args=None)"
        )
