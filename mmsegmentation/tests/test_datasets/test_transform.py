...
def test_resize():
    ...
    transform = dict(
        type="RandomResize", scale=(1280, 800), ratio_range=(1.0, 1.0), resize_type="Resize", keep_ratio=False
    )
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    ...
    transform = dict(
        type="RandomChoiceResize", scales=[(1333, 800), (1333, 400)], resize_type="Resize", keep_ratio=False
    )
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    ...
    transform = dict(
        type="RandomChoiceResize", scales=[128, 256, 512], resize_type="ResizeShortestEdge", max_size=1333
    )
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    assert resized_results["img_shape"][0] in [128, 256, 512]

    transform = dict(type="RandomChoiceResize", scales=[512], resize_type="ResizeShortestEdge", max_size=512)
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    ...
    # img shape: (288, 512, 3)
    transform = dict(type="Resize", scale=None, scale_factor=(0.5, 2.0), keep_ratio=True)
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    ...
    transform = dict(type="Resize", scale_factor=min_size_ratio, keep_ratio=True)
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
    assert resized_results["img_shape"] == (640, 1138)
    ...
    results["scale_factor"] = 1.0
    min_size_ratio = max(640 / img.shape[0], 640 / img.shape[1])
    transform = dict(type="Resize", scale=(2560, 640), scale_factor=min_size_ratio, keep_ratio=True)
    resize_module = TRANSFORMS.build(transform)
    resized_results = resize_module(results.copy())
...
def test_random_rotate_flip():
    ...
    transform = dict(type="RandomRotFlip", flip_prob=1.0, rotate_prob=0, degree=20)
    rot_flip_module = TRANSFORMS.build(transform)

    results = dict()
    img = mmcv.imread(
        osp.join(osp.dirname(__file__), "../data/pseudo_synapse_dataset/img_dir/case0005_slice000.jpg"), "color"
    )
    original_img = copy.deepcopy(img)
    seg = np.array(
        Image.open(osp.join(osp.dirname(__file__), "../data/pseudo_synapse_dataset/ann_dir/case0005_slice000.png"))
    )
    original_seg = copy.deepcopy(seg)
    results["img"] = img
    results["gt_semantic_seg"] = seg
    ...
    transform = dict(type="RandomRotFlip", flip_prob=0, rotate_prob=1.0, degree=20)
    rot_flip_module = TRANSFORMS.build(transform)
    ...
    assert str(transform) == "{'type': 'RandomRotFlip'," " 'flip_prob': 0," " 'rotate_prob': 1.0," " 'degree': 20}"
...
def test_photo_metric_distortion():
    ...
    pipeline = PhotoMetricDistortion(saturation_range=(1.0, 1.0))
    results = pipeline(results)
...
def test_CLAHE():
    ...
    for i in range(original_img.shape[2]):
        converted_img[:, :, i] = mmcv.clahe(np.array(original_img[:, :, i], dtype=np.uint8), 2, (8, 8))
...
def test_cutout():
    ...
    # test n_holes
    with pytest.raises(AssertionError):
        transform = dict(type="RandomCutOut", prob=0.5, n_holes=(5, 3), cutout_shape=(8, 8))
        TRANSFORMS.build(transform)
    with pytest.raises(AssertionError):
        transform = dict(type="RandomCutOut", prob=0.5, n_holes=(3, 4, 5), cutout_shape=(8, 8))
        TRANSFORMS.build(transform)
    ...
    with pytest.raises(AssertionError):
        ...
        TRANSFORMS.build(transform)
    with pytest.raises(AssertionError):
        transform = dict(type="RandomCutOut", prob=0.5, n_holes=1, cutout_shape=(2, 2), cutout_ratio=(0.4, 0.4))
        TRANSFORMS.build(transform)
    # test seg_fill_in
    with pytest.raises(AssertionError):
        transform = dict(type="RandomCutOut", prob=0.5, n_holes=1, cutout_shape=(8, 8), seg_fill_in="a")
        TRANSFORMS.build(transform)
    with pytest.raises(AssertionError):
        transform = dict(type="RandomCutOut", prob=0.5, n_holes=1, cutout_shape=(8, 8), seg_fill_in=256)
        TRANSFORMS.build(transform)
    ...
    transform = dict(
        type="RandomCutOut", prob=1, n_holes=1, cutout_ratio=(0.8, 0.8), fill_in=(255, 255, 255), seg_fill_in=255
    )
    cutout_module = TRANSFORMS.build(transform)
    cutout_result = cutout_module(copy.deepcopy(results))
    assert cutout_result["img"].sum() > img.sum()
...
def test_biomedical3d_random_crop():
    ...
    from mmseg.datasets.transforms import LoadBiomedicalAnnotation, LoadBiomedicalImageFromFile

    results = dict()
    results["img_path"] = osp.join(osp.dirname(__file__), "../data", "biomedical.nii.gz")
    transform = LoadBiomedicalImageFromFile()
    results = transform(copy.deepcopy(results))

    results["seg_map_path"] = osp.join(osp.dirname(__file__), "../data", "biomedical_ann.nii.gz")
    transform = LoadBiomedicalAnnotation()
    results = transform(copy.deepcopy(results))

    d, h, w = results["img_shape"]
    transform = dict(type="BioMedical3DRandomCrop", crop_shape=(d - 20, h - 20, w - 20), keep_foreground=True)
    transform = TRANSFORMS.build(transform)
    ...
    transform = dict(type="BioMedical3DRandomCrop", crop_shape=(d - 20, h - 20, w - 20), keep_foreground=False)
    transform = TRANSFORMS.build(transform)
    crop_results = transform(results)
    assert crop_results["img"].shape[1:] == (d - 20, h - 20, w - 20)
    assert crop_results["img_shape"] == (d - 20, h - 20, w - 20)
...
def test_biomedical_gaussian_noise():
    ...
    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalGaussianNoise", prob=0.2, mean=0.5, std=-0.5)
        TRANSFORMS.build(transform)

    transform = dict(type="BioMedicalGaussianNoise", prob=1.0)
    noise_module = TRANSFORMS.build(transform)
    assert str(noise_module) == "BioMedicalGaussianNoise" "(prob=1.0, " "mean=0.0, " "std=0.1)"

    transform = dict(type="BioMedicalGaussianNoise", prob=1.0)
    noise_module = TRANSFORMS.build(transform)
    results = dict(img_path=osp.join(osp.dirname(__file__), "../data/biomedical.nii.gz"))
...
def test_biomedical_gaussian_blur():
    ...
    with pytest.raises(AssertionError):
        ...
        TRANSFORMS.build(transform)
    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalGaussianBlur", prob=1.0, sigma_range=0.6)
    ...
    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalGaussianBlur", prob=1.0, sigma_range=(15, 8, 9))
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalGaussianBlur", prob=1.0, sigma_range="0.16")
        TRANSFORMS.build(transform)

    transform = dict(type="BioMedicalGaussianBlur", prob=1.0, sigma_range=(0.7, 0.8))
    smooth_module = TRANSFORMS.build(transform)
    assert (
        str(smooth_module) == "BioMedicalGaussianBlur(prob=1.0, "
    ...
    transform = dict(type="BioMedicalGaussianBlur", prob=1.0, different_sigma_per_axis=False)
    smooth_module = TRANSFORMS.build(transform)
...
def test_BioMedicalRandomGamma():
    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=-1, gamma_range=(0.7, 1.2))
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.2, gamma_range=(0.7, 1.2))
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7))
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 0.2, 0.3))
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), invert_image=1)
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), per_channel=1)
        TRANSFORMS.build(transform)

    with pytest.raises(AssertionError):
        transform = dict(type="BioMedicalRandomGamma", prob=1.0, gamma_range=(0.7, 2), retain_stats=1)
        TRANSFORMS.build(transform)
...
def test_BioMedical3DPad():
    ...
    data_info2 = dict(img=np.random.random((8, 6, 4, 4)), gt_seg_map=np.random.randint(0, 2, (6, 4, 4)))
...
def test_biomedical_3d_flip():
    ...
    transform = dict(type="BioMedical3DRandomFlip", prob=1, axes=(1, 2), swap_label_pairs=[(0, 1)])
    transform = TRANSFORMS.build(transform)
    results = data.copy()
    results = transform(results)
